...
```env
NETEASE_API_BASE_URL=http://localhost:4000   # Required — NeteaseCloudMusicApi server URL
NETEASE_API_KEY=xxx                          # Optional — API key
NETEASE_API_SECRET=xxx                       # Optional — API secret
```
...
- **PlayerContext** (`src/contexts/player-context.tsx`) — 14 action types: PLAY_SONG, SET_PLAYLIST, TOGGLE_PLAY, PAUSE, PLAY, NEXT, PREV, SET_VOLUME, SET_PROGRESS, SET_DURATION, SET_PLAY_MODE, REMOVE_SONG, PLAY_NEXT, HYDRATE_STATE. State includes a `priorityQueue` for "play next" functionality. Manages audio playback via a global `<audio>` element.
...
Vitest with jsdom environment. Setup file: `vitest-setup.ts` (imports `@testing-library/jest-dom`). Test files live alongside source in `__tests__/` directories. Uses `@testing-library/react` for component tests.
